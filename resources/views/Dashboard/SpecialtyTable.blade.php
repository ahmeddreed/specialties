        ...
        <div class="card">
            <div class="card-title  px-3">
                <div id="addSpeciaty">
                    ...
                    <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <h5 class="modal-title" id="exampleModalLabel"><h5 class="text-center"> اضافة تخصصات</h5></h5>
                            <button type="button" class="btn btn-light" data-bs-dismiss="modal" aria-label="Close">X</button>
                            </div>
                            <div class="modal-body">

                                <form dir="rtl" class="row g-3 needs-validation mx-auto" method="POST" action="{{ route('createSpeciaty') }}">
                                    @csrf
                                    @method("post")
                                    <div class="col-12">
                                      <label for="yourName" class="form-label fs-5 ">اسم التخصص</label>
                ...
                <div class="d-flex justify-content-end">
                    <form class="d-flex col-lg-3  mx-5" dir="ltr"  data-aos="fade-up" action="{{ route("searchSpecialtyTable") }}" method="POST">
                        @method("post")
                        @csrf
            ...
            <div class="card-body">
             ...
             @endphp
              ...
              <table class="table table-hover">
                ...
                <tbody>
                    @foreach($data_specialty as $data)
                    <tr>
                        <th scope="row">{{ ++$i }}</th>
                        <td>{{ $data->name }}</td>
                        <td>{{ $data->title }}</td>
                        ...
                        <td class="d-flex">
                            @if (auth()->user()->role == "manager")
                                ...
                                <div class="modal fade" id="exampleModalEdit{{ $data->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel"><h5 class="text-center"> تعديل تخصصات</h5></h5>
                                        <button type="button" class="btn btn-light" data-bs-dismiss="modal" aria-label="Close">X</button>
                                        </div>
                                        <div class="modal-body">

                                            <form dir="rtl" class="row g-3 needs-validation mx-auto" method="POST" action="{{ route('updateSpecialty',['id'=>$data->id]) }}">
                                                @csrf
                                                @method("put")
                                                <div class="col-12">
                                                <label for="yourName" class="form-label fs-5 ">اسم التخصص</label>
                                                ...
                                                <div class="mb-3">
                                                    <label for="exampleFormControlTextarea1" class="form-label">التفاصيل</label>
                                                    <textarea class="form-control" id="editor2" name="detail">{!! $data->details !!}</textarea>
                                                    @error('detail')
                                                    <small class="text-danger">
                                                        {{ $message }}
                                                    </small>
                                ...
                                <div class="modal fade" id="exampleModalDelete{{ $data->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route("destroySpecialty",["id"=>$data->id]) }}" method="POST">
                                            @method('delete')
                                            @csrf
